	...
	private void create(AbstractShape s) {
		...
		do {
			...
			if ((s.getClass() == Square.class) ||
				...
				r.height = r.width = (int)Math.round(Math.random() * getMaxSize(s) * s.getParent().getScale());
			} else {
				r.width = (int)Math.round(Math.random() * getMaxSize(s) * s.getParent().getScale());
				r.height = (int)Math.round(Math.random() * getMaxSize(s) * s.getParent().getScale());
			}
			r.x = (int)Math.round(Math.random() * (s.getParent().getWidth() - r.width)) + s.getParent().getX();
			r.y= (int)Math.round(Math.random() * (s.getParent().getHeight() - r.height)) + s.getParent().getY();
		...
		if (!(counter < 100 )) {
			ShapeApplet.setOutputText(s.getClass().toString() + " can not be placed");
